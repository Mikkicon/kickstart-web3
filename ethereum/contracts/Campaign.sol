...
contract Campaign {
    ...
    function approveRequest(uint256 requestIdx) public contributorOnly {
        require(requests[requestIdx].status == Status.pending);
        require(requests[requestIdx].approved[msg.sender] == false);

        requests[requestIdx].approved[msg.sender] = true;
        requests[requestIdx].amountApproved += 1;
    }

    function finalizeRequest(uint256 requestIdx) public managerOnly {
        require(address(this).balance >= requests[requestIdx].amount);

        uint256 amountApproved = requests[requestIdx].amountApproved;
